...
struct TestView: View {
    ...
    func getRoutes() {
        ...
        Task {
            do {
                let routes = try await service.getRoutes(from: Towns.moscow, to: Towns.tumen, limit: 1)
                print(routes)
            } catch {
                print("Ошибка  \(error)")
            }
    ...
    func getScheduleFor() {
        ...
        Task {
            do {
                let schedule = try await service.getScheduleFor(station: TownStations.svo, limit: 5, date: "2024-03-12")
                print(schedule)
            } catch {
                print("Ошибка  \(error)")
            }
